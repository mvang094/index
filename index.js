...
speed = 500; //Changed speed to 500
console.log(speed); //New output
...
let faveSnack = "Cheetos"; //Variable to hold favorite snack
console.log(faveSnack); //Output
...
const private = true; //Uunchanging variable to hold private network
...
let netflix = 15, hulu = 7, disney = 5, youTube = 10, peacock = 10; //Variables for each subscription
...
let total = netflix + hulu + disney + youTube + peacock; //Variable to hold sum of subscriptions
...
console.log(hulu, peacock, netflix); //Display the old prices of streaming networks

hulu = hulu - 7; //Canceled Hulu
peacock -= 5; //Downgraded service from $10 to $5/month
netflix -= 7; //Downgraded service from $15 to $8/month

console.log(hulu, peacock, netflix); //Display changed prices of streaming networks
...
let newTotal = netflix + hulu + peacock + youTube + disney; //New total for changed subscriptions
...
let savings; //The percent of savings after the subscriptions changed
let quotient; //To hold values to calculate savings

quotient = newTotal / total;
savings = 100 - (quotient*100);

console.log("The total percent savings is: ", savings);
